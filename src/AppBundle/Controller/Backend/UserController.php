...
class UserController extends Controller {
            ...
            if ($form->isValid()) {
                ...
                if ($error) {
                    ...
                    if ($file) {
                        ...
                        $file->move($this->getParameter('upload_files'), $fileName);
    ...
    public function editAction(Request $request, User $user) {
        ...
        $oldPicture = $user->getAvatarPath();
        $em = $this->getDoctrine()->getManager();
        $userData = $this->get("session")->get("userData");
        $user_id = $this->getUser()->getId();

        $userObj = $this->getDoctrine()->getRepository('AppBundle:User')->findOneBy(array(
            "id" => $request->get('id')
        ));
