...
class SessionHelper
{
    ...
    public function send_sms( $phone = "" , $message = "" , $connection = "" ){
		if ( $connection == '/SMPP/PCS-RT-444-01' || $connection == '/BC/PCS-GT-MT' )
		{
			$short_code = '50257005324';
			$connection = '/SMPP/PCS-RT-444-01';
		}
		if($connection == "/SMPP/COMCEL-RT-333-01" ||  $connection == "/SMPP/COMCEL-RT-444-01" ||  $connection == "/SMPP/COMCEL-RT-4056-01" ||  $connection == "/SMPP/COMCEL-RT-2013" ){
			$url  = 'http://192.168.10.203:13013/cgi-bin/sendsms?user=sms&password=fuakpe&account=1&to='.$phone.'&from=57005324&smsc='. urlencode($connection).'&text='. urlencode($message);         
		    $url .= "&dlr-mask=31";
		}else{
			$url  = 'http://192.168.10.204:13013/cgi-bin/sendsms?user=tester&password=foobar&account=1&to='. $phone .'&from=57005324&smsc='. urlencode($connection) .'&text='. urlencode($message);
		}
		echo $url;
		echo $response_url = $this->open_url($url);
		return $response_url;
	}
